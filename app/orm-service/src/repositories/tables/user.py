...
from src.schemas.user import UserManagerCreate, UserManagerCreateAPI, UserManagerUpdate
...
class UserManagerRepository(UserBaseRepository, CRUDRepository[User, UserManagerCreate, UserManagerUpdate]):
    def __init__(self) -> None:
        super().__init__(User)

    async def create_extended(
        self, session: AsyncSession, raw_data: UserManagerCreateAPI, avatar_path: UploadFile | None
    ) -> User:
        role: Role = await super()._get_role(session, "manager")
        obj_in = UserManagerCreate(
            first_name=raw_data.first_name,
            last_name=raw_data.last_name,
            phone=raw_data.phone,
            email=raw_data.email,
            password_hash=hash_password(raw_data.password_hash),
            role_id=role.id,
        )
    ...
    async def update_by_id_extended(
        self, session: AsyncSession, id: UUID, obj_in: UserManagerUpdate, avatar_path: UploadFile | None
    ) -> User:
        if avatar_path:
